...
class CustomDatasetFromImages(Dataset):
    def __init__(self, csv_path= ISIC_path + "/ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv", \
        ...
        self.title_info = pd.read_csv(csv_path, nrows=1)
        self.headers = self.title_info.columns.tolist()[1:]
        ...
        #self.pairs = list(zip(self.headers, self.data_info.iloc[0]))
        # Calculate len
    ...
    def __getitem__(self, index):
        ...
        dic = {index: value for index, value in enumerate(self.headers)}

        return (dic, img_as_img, single_image_label)
...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for i, (dic, data, label) in enumerate(d):
            label_class = dic[label]
...
class SubDataset:
    ...
    def __getitem__(self,i):
        ...
        img = self.transform(self.sub_meta[i][1])
